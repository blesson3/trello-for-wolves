...
describe('SETUP | Test Preparation and Setup', () => {
  ...
  describe('ORG-P | Organization POST requests', () => {
    ...
    beforeAll(() => {
      if (!global.resources.org) {
        global.resources.org = {};
      }
    });
    ...
    test('ORG-P-02-T01 | uploads a Logo to an Organization', () => {
      const logoFile = fs.createReadStream(`${assetsDir}/logo.jpg`);
      expect.assertions(1);
      return expect(trello.organizations(orgId).uploadLogo(logoFile)).resolves.toBeDefined();
    });
    ...
    test.skip('ORG-P-03-T01 | adds Tags to an Organization', () => {
      expect.assertions(1);
      return expect(trello.organizations(orgId).addTags('')).resolves.toBeDefined();
    });

    test('ORG-U-04-T01 | adds an associated Member', async () => {
  ...
  describe('BRD-P | Board POST requests', () => {
    ...
    test.skip('BRD-P-02-T01 | generates a calendar key for a Board', () => {
      expect.assertions(1);
      return expect(trello.boards(boardId).generateCalendarKey()).resolves.toBeDefined();
    });
    ...
    test.skip('BRD-P-04-T01 | generates an email key for a Board', () => {
      expect.assertions(1);
      return expect(trello.boards(boardId).generateEmailKey()).resolves.toBeDefined();
    });
    ...
    test.skip('BRD-P-05-T01 | adds Tags to a Board', () => {
      expect.assertions(1);
      return expect(trello.boards(boardId).addTags('[tag]')).resolves.toBeDefined();
    });

    test('BRD-P-06-T01 | adds a Label to a Board', async () => {
    ...
    test('BRD-P-06-T01 | adds a List to a Board', async () => {
    ...
    test('BRD-P-08-T01 | marks Board as viewed', () => {
      expect.assertions(1);
      return expect(trello.boards(boardId).markAsViewed()).resolves.toBeDefined();
    });

    // @fix: Figure out why this isn't working.
    test.skip('BRD-P-09-T01 | adds a PowerUp to a Board', () => {
      expect.assertions(1);
      return expect(trello.boards(boardId).addPowerUp('cardAging')).resolves.toBeDefined();
    });
  });
  ...
  describe('LBL-P | Label POST requests', () => {
    ...
    test('LBL-P-01-T01 | creates a Green Label', async () => {
    ...
    test('LBL-P-01-T02 | creates a Blue Label', async () => {
  ...
  describe('LST-P | List POST requests', () => {
    ...
    test('LST-P-03-T01 | adds a Card to a List with no description', async () => {
    ...
    test('LST-P-03-T02 | adds a Card to a List with description and label', async () => {
    ...
    test('LST-P-04-T01 | moves all Cards to a List', () => {
      const boardId = global.resources.board.id;
      expect.assertions(1);
      return expect(
    ...
    test('LST-P-02-T01 | archives all Cards on a List', () => {
      expect.assertions(1);
      return expect(trello.lists(listId).archiveAllCards()).resolves.toBeDefined();
    });
  });
  ...
  describe('CAR-P | Card POST Requests', () => {
    ...
    test('CAR-P-02-T01 | adds a Comment to a Card', async () => {
    ...
    test('CAR-P-02-T02 | adds a second Comment to a Card', async () => {
    ...
    test('CAR-P-03-T01 | uploads an image Attachment to a Card', async () => {
    ...
    test('CAR-P-03-T02 | uploads a plain text Attachment to a Card', async () => {
    ...
    test('CAR-P-03-T03 | uploads an Attachment as a URL to a Card', async () => {
      expect.assertions(1);
      const response = await trello.cards(cardId).attachments().uploadAttachment({
    ...
    test('CAR-P-06-T01 | adds a Checklist to a Card', async () => {
    ...
    test('CAR-P-06-T02 | copies a Checklist on a Card', async () => {
    ...
    test('CAR-P-04-T01 | adds a Check Item to a Checklist on a Card with default position', async () => {
    ...
    test('CAR-P-04-T02 | adds a Check Item to a Checklist on a Card with numbered position', async () => {
      if (!global.resources.card.checklist) throw new Error('Checklist not found.');
      const checklistId = global.resources.card.checklist.id;
    ...
    test('CAR-P-04-T03 | adds a Check Item to a Checklist on a Card with named position', async () => {
      if (!global.resources.card.checklist) throw new Error('Checklist not found.');
      const checklistId = global.resources.card.checklist.id;
    ...
    test('CAR-P-05-T01 | converts a Check Item to a Card', async () => {
      if (!global.resources.card.checklist) throw new Error('Checklist not found.');
      const checklistId = global.resources.card.checklist.id;
    ...
    test('CAR-P-07-T01 | associates a Label with a Card', () => {
      if (!global.resources.labels.length) throw new Error('Label not found.');
      const labelId = global.resources.labels[0].id;
      expect.assertions(1);
      return expect(trello.cards(cardId).associateLabel(labelId)).resolves.toBeDefined();
    });
    ...
    test.skip('CAR-P-08-T01 | associates a Member with a Card', () => {
      ...
      expect.assertions(1);
      return expect(trello.cards(cardId).associateMember(memberId)).resolves.toBeDefined();
    });

    test('CAR-P-09-T01 | adds a Label to a Card', async () => {
    ...
    test('CAR-P-10-T01 | marks associated Notifications as read', () => {
      expect.assertions(1);
      return expect(trello.cards(cardId).markAssociatedNotificationsRead()).resolves.toBeDefined();
    });
    ...
    test.skip('CAR-P-11-T01 | updates the Members Voted on a Card', () => {
      ...
      const memberId = global.resources.member.id;
      expect.assertions(1);
      return expect(trello.cards(cardId).membersVoted(memberId).updateVote(true)).resolves.toBeDefined();
    });

    test('CAR-P-12-T01 | adds a Sticker with the Check image to a Card', async () => {
    ...
    test('CAR-P-12-T02 | adds a Sticker with the Heart image to a Card', async () => {
  ...
  describe('CHK-P | Checklist POST requests', () => {
    ...
    test('CHK-P-02-T01 | adds an unchecked Check Item to a Checklist', async () => {
    ...
    test('CHK-P-02-T02 | adds a checked Check Item to a Checklist', async () => {
